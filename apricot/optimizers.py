...
from .utils import PriorityQueue, check_random_state
    ...
    def select(self, X, k, sample_cost=None):
        ...
        while cost < k:
            idxs = self.random_state.choice(
                self.function.idxs, replace=False, size=min(subset_size, self.function.idxs.shape[0])
            )
...
class SampleGreedy(BaseOptimizer):
    ...
    def select(self, X, k, sample_cost=None):
        ...
        idxs = self.random_state.choice(self.function.idxs, replace=False, size=n - subset_size)

        if isinstance(self.optimizer, str):
            optimizer = OPTIMIZERS[self.optimizer](function=self.function, verbose=self.verbose)
        elif not isinstance(self.optimizer, BaseOptimizer):
            raise ValueError("optimizer must be either a string or an " / "optimizer object.")
        else:
...
class SieveGreedy(BaseOptimizer):
    ...
    def __init__(self, function=None, epsilon=0.01, random_state=None, n_jobs=None, verbose=False):
        self.epsilon = epsilon
        self.n_seen_ = 0
        self.thresholds = [1]
        self.max_gain = -1
