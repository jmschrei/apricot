...
class FacilityLocationSelection(BaseGraphSelection):
        ...
        if self.initial_subset is None:
            ...
            if not self.sparse:
                for i in self.initial_subset:
                    self.current_values = numpy.maximum(X_pairwise[i], self.current_values).astype("float64")
            else:
                for i in self.initial_subset:
                    self.current_values = numpy.maximum(X_pairwise[i].toarray()[0], self.current_values).astype(
                        "float64"
                    )
        else:
            raise ValueError(
    ...
    def _calculate_gains(self, X_pairwise, idxs=None):
        ...
        if self.sparse:
            self.calculate_gains_(
                X_pairwise.data, X_pairwise.indices, X_pairwise.indptr, gains, self.current_values, idxs
            )
        else:
    ...
    def _select_next(self, X_pairwise, gain, idx):
        ...
        if self.sparse:
            self.current_values = numpy.maximum(X_pairwise.toarray()[0], self.current_values)
        else:
            self.current_values = numpy.maximum(X_pairwise, self.current_values)
