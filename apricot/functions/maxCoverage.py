...
from ..optimizers import ApproximateLazyGreedy, LazyGreedy
...
def calculate_gains_sparse(dtypes, parallel, fastmath, cache):
    ...
    def calculate_gains_sparse_(X_data, X_indices, X_indptr, gains, current_values, threshold, idxs):
        for i in prange(idxs.shape[0]):
            idx = idxs[i]
            start = X_indptr[idx]
            end = X_indptr[idx + 1]
...
def calculate_gains_sieve(dtypes, parallel, fastmath, cache):
    ...
    def calculate_gains_sieve_(
        X, k, current_values, selections, gains, total_gains, max_values, n_selected, thresh, idxs
    ):
        n = X.shape[0]
        ...
        for j in prange(d):
            for i in range(n):
                ...
                gain = numpy.fmin(current_values[j] + X[i], thresh).sum() - total_gains[j]
...
def calculate_gains_sieve_sparse(dtypes, parallel, fastmath, cache):
    @njit(dtypes, parallel=parallel, fastmath=fastmath, cache=cache)
    def calculate_gains_sieve_sparse_(
        X_data,
        X_indices,
        X_indptr,
        k,
        ...
        for j in prange(d):
            for i in range(idxs.shape[0]):
                ...
                for l in range(start, end):
                    m = X_indices[l]
                    gain += min(current_values[j, m] + X_data[l], thresh) - current_values[j, m]

                if gain > threshold:
                    for l in range(start, end):
                        m = X_indices[l]
                        current_values[j, m] = min(current_values[j, m] + X_data[l], thresh)
...
class MaxCoverageSelection(BaseSelection):
    ...
    def _initialize(self, X):
        ...
        if self.initial_subset is None:
            ...
            self.current_values = numpy.fmin(self.threshold, self.initial_subset.sum(axis=0).astype("float64"))
        ...
        self.calculate_sieve_gains_ = calculate_sieve_gains_(dtypes_, True, True, False)
    ...
    def _calculate_gains(self, X, idxs=None):
        ...
        if self.sparse:
            self.calculate_gains_(X.data, X.indices, X.indptr, gains, self.current_values, self.threshold, idxs)
        else:
            self.calculate_gains_(X, gains, self.current_values, self.threshold, idxs)
    ...
    def _calculate_sieve_gains(self, X, thresholds, idxs):
        ...
        if self.sparse:
            self.calculate_sieve_gains_(
                ...
                X.indptr,
                self.n_samples,
                self.sieve_current_values_,
                self.sieve_selections_,
                self.sieve_gains_,
                self.sieve_total_gains_,
                thresholds,
                self.sieve_n_selected_,
                self.threshold,
                idxs,
            )
        else:
    ...
    def _select_next(self, X, gain, idx):
        ...
        if self.sparse:
            self.current_values = numpy.fmin(self.threshold, self.current_values + X.toarray()[0])
        else:
            self.current_values = numpy.fmin(self.threshold, self.current_values + X)
