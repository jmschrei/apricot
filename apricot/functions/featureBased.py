...
def calculate_gains_sparse(func, dtypes, parallel, fastmath, cache):
    @njit(dtypes, parallel=parallel, fastmath=fastmath, cache=cache)
    def calculate_gains_sparse_(X_data, X_indices, X_indptr, gains, current_values, current_concave_values, idxs):
        for i in prange(idxs.shape[0]):
            idx = idxs[i]
            start = X_indptr[idx]
            end = X_indptr[idx + 1]
            ...
            for j in range(start, end):
                k = X_indices[j]
                gains[i] += func(X_data[j] + current_values[k]) - current_concave_values[k]
...
def calculate_gains_sieve(func, dtypes, parallel, fastmath, cache):
    @njit(dtypes, parallel=parallel, fastmath=fastmath, cache=cache)
    def calculate_gains_sieve_(X, k, current_values, selections, gains, total_gains, max_values, n_selected, idxs):
        n = X.shape[0]
...
def calculate_gains_sieve_sparse(func, dtypes, parallel, fastmath, cache):
    ...
    def calculate_gains_sieve_sparse_(
        X_data, X_indices, X_indptr, k, current_values, selections, gains, total_gains, max_values, n_selected, idxs
    ):
        d = max_values.shape[0]
        ...
        for j in prange(d):
            for i in range(idxs.shape[0]):
                ...
                for l in range(start, end):
                    m = X_indices[l]
                    gain += func(current_values[j, m] + X_data[l]) - func(current_values[j, m])
...
class FeatureBasedSelection(BaseSelection):
    ...
    def _initialize(self, X):
        ...
        if self.optimizer in (LazyGreedy, ApproximateLazyGreedy, "lazy", "approximate-lazy"):
            self.calculate_gains_ = calculate_gains_(self.concave_func, dtypes_, False, True, False)
        else:
            self.calculate_gains_ = calculate_gains_(self.concave_func, dtypes_, True, True, False)
        ...
        self.calculate_sieve_gains_ = calculate_sieve_gains_(self.concave_func, dtypes_, True, True, False)
    ...
    def _calculate_gains(self, X, idxs=None):
        ...
        if self.sparse:
            self.calculate_gains_(
                X.data, X.indices, X.indptr, gains, self.current_values, self.current_concave_values, idxs
            )
        else:
