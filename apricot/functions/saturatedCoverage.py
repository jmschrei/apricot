...
class SaturatedCoverageSelection(BaseGraphSelection):
        ...
        if self.initial_subset is None:
            ...
            if not self.sparse:
                for i in self.initial_subset:
                    self.current_values = numpy.minimum(self.max_values, self.current_values + X_pairwise[i])
            else:
                for i in self.initial_subset:
                    self.current_values = numpy.minimum(
                        self.max_values, self.current_values + X_pairwise[i].toaray()[0]
                    )
        else:
            raise ValueError(
    ...
    def _calculate_gains(self, X_pairwise, idxs=None):
        ...
        if self.sparse:
            ...
            select_next(X_pairwise, gains, self.current_values, self.max_values, idxs)
    ...
    def _select_next(self, X_pairwise, gain, idx):
        ...
        if self.sparse:
            self.current_values = numpy.minimum(self.max_values, X_pairwise.toarray()[0] + self.current_values)
        else:
