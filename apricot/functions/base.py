...
from ..optimizers import OPTIMIZERS, BaseOptimizer, SieveGreedy
from ..utils import _calculate_pairwise_distances, check_random_state
...
class BaseSelection:
    ...
    def fit(self, X, y=None, sample_weight=None, sample_cost=None):
        ...
        if isinstance(self.optimizer, str):
            optimizer = OPTIMIZERS[self.optimizer](
                function=self, verbose=self.verbose, random_state=self.random_state, **self.optimizer_kwds
            )
        else:
    ...
    def partial_fit(self, X, y=None, sample_weight=None, sample_cost=None):
        ...
        if not isinstance(self.optimizer, SieveGreedy):
            self.optimizer = OPTIMIZERS["sieve"](
                function=self, verbose=self.verbose, random_state=self.random_state, **self.optimizer_kwds
            )
    ...
    def fit_transform(self, X, y=None, sample_weight=None, sample_cost=None):
        ...
        return self.fit(X, y=y, sample_weight=sample_weight, sample_cost=sample_cost).transform(
        ...
        if self.initial_subset is not None:
            if self.initial_subset.ndim == 1:
                ...
                if len(self.initial_subset) + self.n_samples > X.shape[0]:
                    raise ValueError(
                        "When using a mask for the initial subset"
    ...
    def _calculate_sieve_gains(self, X, thresholds, idxs):
        ...
        if self.sieve_current_values_ is None:
            self.sieve_current_values_ = numpy.zeros((l, d), dtype="float64")
            self.sieve_selections_ = numpy.zeros((l, self.n_samples), dtype="int64") - 1
            self.sieve_gains_ = numpy.zeros((l, self.n_samples), dtype="float64") - 1
            self.sieve_n_selected_ = numpy.zeros(l, dtype="int64")
            self.sieve_total_gains_ = numpy.zeros(l, dtype="float64")
            self.sieve_subsets_ = numpy.zeros((l, self.n_samples, self._X.shape[1]), dtype="float64")
        else:
            j = l - self.sieve_current_values_.shape[0]
            if j > 0:
                self.sieve_current_values_ = numpy.vstack(
                    [self.sieve_current_values_, numpy.zeros((j, d), dtype="float64")]
                )
                self.sieve_selections_ = numpy.vstack(
                    [self.sieve_selections_, numpy.zeros((j, self.n_samples), dtype="int64") - 1]
                )
                self.sieve_gains_ = numpy.vstack([self.sieve_gains_, numpy.zeros((j, self.n_samples), dtype="float64")])
                self.sieve_n_selected_ = numpy.concatenate([self.sieve_n_selected_, numpy.zeros(j, dtype="int64")])
                self.sieve_total_gains_ = numpy.concatenate([self.sieve_total_gains_, numpy.zeros(j, dtype="float64")])
                self.sieve_subsets_ = numpy.concatenate(
                    [self.sieve_subsets_, numpy.zeros((j, self.n_samples, self._X.shape[1]), dtype="float64")]
                )
