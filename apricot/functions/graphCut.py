...
def calculate_gains_sieve(dtypes, parallel, fastmath, cache):
    ...
    def calculate_gains_sieve_(
        X, k, current_values, selections, gains, total_gains, max_values, n_selected, row_sums, idxs
    ):
        n, d = X.shape
    ...
    def _calculate_sieve_gains(self, X_pairwise, thresholds, idxs):
        ...
        for i in range(m):
            ...
            sieve_current_values_[i] = _calculate_pairwise_distances(
                self._X, Y=self.sieve_subsets_[i, :l], metric=self.metric
            ).mean(axis=1)
